...
export default function AddRoutineScreen({ route, navigation }) {
  ...
  useLayoutEffect(() => { navigation.setOptions({ title: isEditing ? 'Editar rutina' : 'Nueva rutina' }); }, [isEditing]);
  ...
  function toggleDay(dayName) {
    ...
    } else setDays(p => [...p, { dayName, exercises: [] }]);
  }
  ...
  function toggleExInDay(exercise) {
    setDays(p => p.map(d => {
      ...
      return { ...d, exercises: [...d.exercises, { exerciseId: exercise.id, targetSets: 3, targetReps: 10 }] };
    }));
  }

  function updateTarget(dayName, exerciseId, field, value) {
    setDays(p => p.map(d => d.dayName !== dayName ? d : {
      ...d, exercises: d.exercises.map(e => e.exerciseId !== exerciseId ? e : { ...e, [field]: parseInt(value) || 0 })
    }));
  }

  function removeEx(dayName, exerciseId) {
    setDays(p => p.map(d => d.dayName !== dayName ? d : { ...d, exercises: d.exercises.filter(e => e.exerciseId !== exerciseId) }));
  }

  async function handleSave() {
    if (!name.trim()) { Alert.alert('Error', 'El nombre es obligatorio'); return; }
    const weeksNum = parseInt(weeks);
    if (isNaN(weeksNum) || weeksNum < 1 || weeksNum > 52) { Alert.alert('Error', 'Las semanas deben ser entre 1 y 52'); return; }
    if (days.length === 0) { Alert.alert('Error', 'Agregá al menos un día'); return; }
    if (!days.some(d => d.exercises.length > 0)) { Alert.alert('Error', 'Agregá al menos un ejercicio'); return; }
    setIsSaving(true);
    try {
      const data = { name: name.trim(), weeks: weeksNum, days };
      if (isEditing) await updateRoutine(editingRoutine.id, data); else await saveRoutine(data);
      navigation.goBack();
    ...
    <View style={s.container}>
      <ScrollView style={s.scroll} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.fieldGroup}>
          ...
          <TextInput style={s.input} value={name} onChangeText={setName} placeholder="Ej: Push Pull Legs, Fuerza A/B..." placeholderTextColor={colors.textLight} maxLength={50} />
        </View>
        ...
        <View style={s.fieldGroup}>
          ...
          <View style={s.weeksRow}>
            {[4,6,8,12,16].map(w => (
              ...
              >
                <Text style={[s.weekChipText, weeks === w.toString() && { color: colors.textOnBrand, fontWeight: '700' }]}>{w} sem</Text>
              </TouchableOpacity>
            ))}
            <TextInput
          ...
          <Text style={s.hint}>La rutina durará {parseInt(weeks) || 0} semana{parseInt(weeks) !== 1 ? 's' : ''} en total.</Text>
        </View>
                ...
                <View key={dayName} style={s.dayCard}>
                  ...
                  {(dayData?.exercises || []).length === 0 ? (
                    ...
                    (dayData?.exercises || []).map(ex => {
                      ...
                      const mc = colors.muscleColors[exData.muscleGroup] || colors.muscleColors['Otro'];
                      return (
                        <View key={ex.exerciseId} style={[s.exRow, { borderTopColor: colors.border }]}>
                          <View style={[s.exRowBadge, { backgroundColor: mc.bg }]}>
                            <Text style={[s.exRowBadgeText, { color: mc.text }]}>{exData.muscleGroup.slice(0,2).toUpperCase()}</Text>
                          </View>
                          <Text style={s.exRowName} numberOfLines={1}>{exData.name}</Text>
                          <View style={s.exTargets}>
                            <View style={s.targetField}>
                              <Text style={s.targetLabel}>S</Text>
                              <TextInput style={s.targetInput} value={ex.targetSets?.toString()} onChangeText={v => updateTarget(dayName, ex.exerciseId, 'targetSets', v)} keyboardType="numeric" maxLength={2} />
                            </View>
                            <Text style={s.targetX}>×</Text>
                            <View style={s.targetField}>
                              <Text style={s.targetLabel}>R</Text>
                              <TextInput style={s.targetInput} value={ex.targetReps?.toString()} onChangeText={v => updateTarget(dayName, ex.exerciseId, 'targetReps', v)} keyboardType="numeric" maxLength={3} />
                            </View>
                          </View>
      ...
      <Modal visible={modalVisible} animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={[s.modalContainer, { backgroundColor: colors.background }]}>
          ...
          <View style={[s.modalHeader, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
            ...
            <TouchableOpacity style={[s.modalDoneBtn, { backgroundColor: colors.brand }]} onPress={() => setModalVisible(false)}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              const mc = colors.muscleColors[item.muscleGroup] || colors.muscleColors['Otro'];
              return (
                <TouchableOpacity
