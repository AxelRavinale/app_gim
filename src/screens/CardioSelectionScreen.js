...
export default function CardioSelectionScreen({ navigation }) {
  ...
  return (
    <View style={s.container}>
        ...
        >
          ...
          <View style={s.cardContent}>
            ...
            <Text style={[s.cardDesc, { color: colors.textSecondary }]}>
              {hasGym ? 'Planes y circuitos de tu entrenador' : 'Uníte a un gimnasio para acceder'}
            </Text>
            {!hasGym && (
      ...
      <Text style={[s.footer, { color: colors.textLight }]}>Seleccioná el tipo de cardio</Text>
    </View>
  );
}
...
const makeStyles = (colors) => StyleSheet.create({
  ...
  header:      { flexDirection:'row', alignItems:'center', gap:14, paddingHorizontal:20, paddingBottom:20, borderBottomWidth:0.5, borderBottomColor:colors.border, marginBottom:10 },
  backBtn:     { padding:4 },
  ...
  card:        { borderRadius:20, borderWidth:1.5, padding:22, flexDirection:'row', alignItems:'center', gap:16, overflow:'hidden', position:'relative', shadowColor:'#000', shadowOffset:{width:0,height:4}, shadowOpacity:0.25, shadowRadius:10, elevation:5, minHeight:110 },
