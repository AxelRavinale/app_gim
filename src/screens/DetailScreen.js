...
export default function DetailScreen({ route, navigation }) {
  ...
  async function handleAddSession() {
    if (isCardio) {
      ...
      if (isNaN(duration) || duration <= 0) { Alert.alert('Error', 'Ingresá una duración válida'); return; }
      setIsSaving(true);
      try {
        await addTimeSession(exerciseId, { duration, distance: parseFloat(formDistance) || null });
        setFormDuration(''); setFormDistance('');
        setShowAddModal(false); await loadExercise();
      } catch { Alert.alert('Error', 'No se pudo guardar'); } finally { setIsSaving(false); }
    } else {
      if (seriesInputs.length === 0) { Alert.alert('Error', 'Indicá cuántas series'); return; }
      const validSeries = seriesInputs.map(s => ({ weight: parseFloat(s.weight) || 0, reps: parseInt(s.reps) || 0 }));
      if (validSeries.every(s => s.weight === 0)) { Alert.alert('Error', 'Ingresá al menos un peso'); return; }
      setIsSaving(true);
      try {
        await addWeightSession(exerciseId, validSeries);
        setFormSeries('3'); setSeriesInputs([]);
        setShowAddModal(false); await loadExercise();
      } catch { Alert.alert('Error', 'No se pudo guardar'); } finally { setIsSaving(false); }
    }
  }
      ...
      <Modal visible={showAddModal} animationType="slide" transparent onRequestClose={() => setShowAddModal(false)}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={s.modalOverlay}>
          ...
          <View style={[s.modalContent, { backgroundColor: colors.card }]}>
                ...
                <View style={s.seriesCountRow}>
                  ...
                  {[1, 2, 3, 4, 5].map(n => (
                    <TouchableOpacity key={n}
                      style={[s.seriesCountBtn, { borderColor: colors.border, backgroundColor: colors.cardAlt }, formSeries === n.toString() && { borderColor: colors.brand, backgroundColor: colors.brandLight }]}
                      onPress={() => handleSeriesCountChange(n.toString())}
                    >
                      <Text style={[s.seriesCountBtnText, { color: colors.textSecondary }, formSeries === n.toString() && { color: colors.brand }]}>{n}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
            ...
            <TouchableOpacity style={[s.saveBtn, { backgroundColor: colors.brand }, isSaving && { opacity: 0.6 }]} onPress={handleAddSession} disabled={isSaving}>
              {isSaving ? <ActivityIndicator color={colors.textOnBrand} /> : <Text style={[s.saveBtnText, { color: colors.textOnBrand }]}>Guardar sesión</Text>}
            </TouchableOpacity>
          </View>
...
function FormField({ label, value, onChangeText, placeholder, colors }) {
  return (
    <View style={{ flex: 1 }}>
      ...
      <TextInput
        style={{ backgroundColor: colors.cardAlt, borderRadius: 10, padding: 12, fontSize: 18, fontWeight: '600', color: colors.textPrimary, textAlign: 'center', borderWidth: 1, borderColor: colors.border }}
